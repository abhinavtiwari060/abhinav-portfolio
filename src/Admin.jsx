  ...
  if (!auth) {
    return (
      <div style={{ minHeight: '100vh', background: '#050510', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, sans-serif' }}>
        <div style={{ background: 'rgba(255,255,255,0.05)', padding: 40, borderRadius: 16, border: '1px solid rgba(255,255,255,0.1)', width: '100%', maxWidth: 400 }}>
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13, marginTop: 8 }}>Enter your details to manage your portfolio.</p>
          </div>
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <label style={{ display: 'block', fontSize: 12, marginBottom: 8, color: 'rgba(255,255,255,0.6)' }}>Email ID</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} required style={{ width: '100%', padding: 12, background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, color: '#fff' }} placeholder="admin@abhinav.dev" />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 12, marginBottom: 8, color: 'rgba(255,255,255,0.6)' }}>Password</label>
              <input type="password" value={pass} onChange={e => setPass(e.target.value)} required style={{ width: '100%', padding: 12, background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, color: '#fff' }} placeholder="********" />
            </div>
            {loginError && <div style={{ color: '#ef4444', fontSize: 13 }}>{loginError}</div>}
            <button type="submit" style={{ padding: 12, background: 'linear-gradient(135deg, #a855f7, #06b6d4)', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 600, cursor: 'pointer', marginTop: 8 }}>
              Login to Dashboard
            </button>
            <a href="/" style={{ textAlign: 'center', fontSize: 13, color: 'rgba(255,255,255,0.4)', textDecoration: 'none', marginTop: 8 }}>← Back to site</a>
          </form>
        </div>
      </div>
    );
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 30 }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <a href="/" style={{ padding: '10px 20px', background: 'rgba(255,255,255,0.1)', color: '#fff', borderRadius: 8, textDecoration: 'none' }}>
              ← Return to Site
            </a>
            <button onClick={handleSave} disabled={saving} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 20px', background: 'linear-gradient(135deg, #a855f7, #06b6d4)', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 600 }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 24, paddingBottom: 24, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
              ...
              <div>
                ...
                <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 16px', background: 'rgba(255,255,255,0.1)', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 24, paddingBottom: 24, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', marginBottom: 8 }}>Upload a PDF version of your resume to make it downloadable to visitors.</p>
                <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 16px', background: 'rgba(255,255,255,0.1)', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
              {Object.keys(data.profile || {}).map(field => (
                <div key={field} style={{ gridColumn: (field.includes('Bio') || field.includes('subtitle') || field.includes('about')) ? 'span 2' : 'span 1' }}>
                  ...
                  {(field.includes('Bio') || field.includes('subtitle') || field.includes('about')) ? (
                    <textarea value={data.profile[field]} onChange={(e) => handleProfileEdit(field, e.target.value)} style={{ width: '100%', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', padding: 12, borderRadius: 8, minHeight: 100, resize: 'vertical' }} />
                  ) : (
                    <input type="text" value={data.profile[field]} onChange={(e) => handleProfileEdit(field, e.target.value)} style={{ width: '100%', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', padding: 12, borderRadius: 8 }} />
                  )}
                </div>
              ))}
            </div>
        ...
        {(activeTab === 'projects' || activeTab === 'blogs') && (
          <>
            <button onClick={handleAdd} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 20px', background: 'rgba(255,255,255,0.05)', color: '#fff', border: '1px dashed rgba(255,255,255,0.2)', borderRadius: 8, cursor: 'pointer', marginBottom: 20, width: '100%', justifyContent: 'center' }}>
                      ...
                      <div key={field} style={{ gridColumn: (field === 'description' || field === 'excerpt') ? 'span 2' : 'span 1' }}>
                        ...
                        {(field === 'description' || field === 'excerpt') ? (
                          <textarea
                            ...
                            style={{ width: '100%', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', padding: 12, borderRadius: 8, minHeight: 100, resize: 'vertical' }}
                          />
                        ) : field === 'tags' ? (
                          <input
                            type="text"
                            ...
                            style={{ width: '100%', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', padding: 12, borderRadius: 8 }}
                          />
                        ) : (
                          <input
                            type="text"
